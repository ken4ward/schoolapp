...
Template.studentprofile.events({
	'submit .studentupdate': function (event, template) {
		...
		if (isNotEmpty(firstname) && isNotEmpty(lastname) && isNotEmpty(mobile) && isNotEmpty(num_street)
			&& isNotEmpty(geocomplete) && isNotEmpty(name) && isNotEmpty(country)) {
			Meteor.call('CreateUserRecord', firstname, lastname, mobile, num_street, facebook, geocomplete, 
		        ...
		        if (error) {
		          ...
		          Meteor.call('UserImagesUpload', userId, userimage, function (error, response) {
		            if (error) {
		              ...
		              FlowRouter.redirect('/student/profile');
		            }
		          });
		        }
		      });
